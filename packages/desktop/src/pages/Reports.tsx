...
export default function Reports() {
  ...
  return (
    <div className="p-6">
      ...
      {period === 'custom' && (
        <p className="text-sm text-muted-foreground mb-4">Showing sales from {startDate} to {endDate}</p>
      )}
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(periodData.sales)}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(periodData.profit)}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{periodData.transactions}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatCurrency(periodData.averageSale)}</div>
              </CardContent>
            </Card>
          </div>
                    ...
                    {dailySales.map((sale) => (
                      <div key={sale.date} className="flex items-center justify-between border-b pb-3 last:border-0">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{sale.transactions} transactions</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold">{formatCurrency(sale.total)}</p>
                          <p className="text-sm text-green-600">{formatCurrency(sale.profit)} profit</p>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {paymentMethods.map((pm) => (
                      <div key={pm.method} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="secondary">{pm.percentage}%</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
