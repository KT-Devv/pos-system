...
create unique index product_units_shop_barcode_key on public.product_units(shop_id, barcode) where barcode is not null;
...
create table public.sale_lines (
  ...
  -- `quantity` counts sold units: singles, or packs when unit_id is set. unit_price and unit_cost are
  -- per unit; a pack line took quantity x unit_quantity single items out of stock. The name and size
  -- are copied so receipts stay right if the pack size is later edited or deleted.
  unit_id uuid,
  unit_name text,
